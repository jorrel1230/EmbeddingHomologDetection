    ...
    try:
        ...
        for i in tqdm(range(start_idx, end_idx, batch_size), desc='Evaluating Similarity Search', ncols=100):
            ...
            for j in range(i, batch_end):
                ...
                query_sf = metadata.iloc[j]['sf']  # Ground truth superfamily label
                query_fa = metadata.iloc[j]['fa']  # Ground truth family label
                ...
                for _, row in similar_proteins.iterrows():
                    ...
                    retrieved_sf = row['sf']  # Retrieved protein's superfamily
                    retrieved_fa = row['fa']  # Retrieved protein's family

                    label = 1 if (retrieved_sf == query_sf) or (retrieved_fa == query_fa) else 0  # Homolog or not
...
if __name__ == "__main__":
    ...
    index = faiss.read_index('/scratch/gpfs/jr8867/embeddings/protein-embeddings-contrastive-unnorm/transformed_embeddings.index')
    ...
    output_dir = f'/scratch/gpfs/jr8867/evals/protein-embeddings-contrastive-unnorm/batch-{batch_num}-{num_ranges}'
