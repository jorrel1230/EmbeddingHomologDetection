...
def embed_sequence(sequence, model=None, alphabet=None, batch_converter=None, max_length=1022):
    """
    ...
    """
    # Load model on demand if not provided
    ...
    try:
        # Truncate sequence if too long (same as in embed_seqs.py)
        ...
        token_embeddings = results["representations"][33]  # Extract last hidden layer
        ...
        # Clean up if we created a temporary model
        if temp_model:
...
def search_similar_proteins(query_seq, index, metadata, k=5, model=None, alphabet=None, batch_converter=None):
    """
    ...
    - model, alphabet, batch_converter: Optional model components
    
    Returns:
    - DataFrame: Metadata of similar proteins
    """
    # Embed query sequence
    query_embedding = embed_sequence(query_seq, model, alphabet, batch_converter)
...
def save_results(all_scores, all_labels, batch_num, total_batches, output_dir):
    ...
    plt.title(f'ROC Curve for Protein Homology Detection (Batch {batch_num})')
...
if __name__ == "__main__":
    ...
    processing_batch_size = 5  # Process 5 queries at a time
    ...
    output_dir = f'/scratch/gpfs/jr8867/evals/protein-embeddings-norm/batch-{batch_num}-{num_ranges}'
